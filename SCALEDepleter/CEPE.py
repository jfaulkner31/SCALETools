...
def CELI(fissionable_mats: list,
  ...
  for material_index in fissionable_mats:
    ...
    xenon_current[material_index] = mat_lib.material_dict[material_index].return_iso_atom_dens(iso='xe-135')
  ...
  for step_num in range(num_steps):
    ...
    # 2. Deplete with origen. Get Xe at end of each substep
    # 3. Correct and renormalize power
    # 4. Go back to 1.
    for substep_idx, LI_time in enumerate(LI_times):
      bos_power = power_by_step[step_num]
      eos_power = power_by_step[step_num+1]
      substep_power = bos_power # fix here to interpolate based on xenon and stuffs - placeholder
      bos_mat_lib = time_lib.material_lib_from_step(requested_step=step_num, PC_flag='C')
      substep_time_start = LI_starts[substep_idx]
      substep_time_end = LI_ends[substep_idx]
      makeAndRunOrigen.makeOrigenCEPEFile(fission_mat_ids=fissionable_mats, substep_power=substep_power,
