...
def f33Interpolate(filepath: str, bos_file: str, eos_file: str, times: list, start_time: float, end_time: float, appendThis: str):
  ...
  # Get absolute path of the bash script f71_to_comp.sh
  this_dir = os.path.dirname(__file__)
  bash_f71_path = this_dir + "/../depletion_bash_scripts/interp2files.sh"
  ...
  for idx, time in enumerate(times):
    ...
    f33path = filepath+'/'+f33name # Output path filename
    f33_substep_filepath_list.append(f33path)
    p = subprocess.run(['bash', bash_f71_path, bos_file, eos_file, str(start_time), str(end_time), str(time), f33path],
...
def makeOrigenCELIFile(fiss_mat_id: int,
  ...
  for idx, start_time in enumerate(LI_starts):
    ...
    this_file.write('  }\n')
    this_file.write('  power='+str([float(val) for val in POWER_VECTOR])+' % MW\n')
