...
def CELI(fissionable_mats: list,
  ...
  for step_num in range(num_steps):
    ...
    for idx, fiss_mat_id in enumerate(fissionable_mats):
      ...
      _ = makeStdCmp.makeStdCmpFromF71(materialNumber=fiss_mat_id, temperature=temperature, filename=origen_f71_loc, dictionaryFilename=addnuxdict, outputFilename=stdCmpFilename, tmpdir=tmpdir, scale_version=scale_version)
    ...
    for ci in range(corrector_iterations):
      ...
      for idx, fiss_mat_id in enumerate(fissionable_mats):
        ...
        _ = makeStdCmp.makeStdCmpFromF71(materialNumber=fiss_mat_id, temperature=temperature, filename=blended_f71_filename, dictionaryFilename=addnuxdict, outputFilename=stdCmpFilename, tmpdir=tmpdir, scale_version=scale_version)
