...
def copy_files_from_temp(tmpdir: str, step_num: int, mcf33dir: str, appendThis=''):
  ...
  for filename in all_files:
    if ("f33" in filename) & ('mix' in filename):
      # SCALE 6.3.X naming style for f33 files!
    ...
    elif ("ft33f001" in filename) & ("mix" in filename):
      # SCALE 6.2.4 naming style for f33 files!
      # EXAMPLE: ft33f001.mix0101
      index_x = filename.find('x')
      number = int(filename[index_x+1::])
      new_filename = str(number)+'.f33'
